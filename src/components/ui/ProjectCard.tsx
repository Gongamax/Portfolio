...
const ProjectCard = ({ project }: ProjectCardProps) => {
  const gradientClass =
    projectGradients[project.id as keyof typeof projectGradients];

  const renderProjectHeader = () => {
    if (project.videoDemo) {
      return (
        <div className="text-center transition-transform duration-300">
          <div className="w-20 h-20 bg-white/20 rounded-full flex items-center justify-center mb-4 mx-auto border border-white/30 transition-all duration-200">
  ...
  return (
    <div className="group bg-white rounded-3xl shadow-lg border border-gray-100 overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
      {/* Project Header */}
      <div
        className={`h-64 relative overflow-hidden bg-gradient-to-br ${gradientClass}`}
      >
        ...
        <div className="flex gap-3">
          {project.liveUrl && (
            <a
              ...
              className="group/btn inline-flex items-center space-x-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold px-6 py-3 rounded-2xl hover:from-blue-700 hover:to-indigo-700 transition-all duration-200"
            >
              <span>View Live</span>
              <svg
          ...
          {project.videoDemo && project.demoVideoUrl && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="group/btn inline-flex items-center space-x-2 bg-gradient-to-r from-red-600 to-pink-600 text-white font-bold px-6 py-3 rounded-2xl hover:from-red-700 hover:to-pink-700 transition-all duration-200"
            >
